...
console.log(window); // See if we have previously saved a state and if not, insert an empty array

var initialState = JSON.parse(localStorage.getItem(notesStorageKey) || '{"notes" : []}'); // This is used to reset the state if something is saved as undefined
// initialState = {notes:[]}
// Create the store
...
var notesApp = function notesApp() {
  ...
  switch (action.type) {
    ...
    case "ADD_NOTE":
      ...
      var noteTextIndex = Math.floor(Math.random() * NoteMessages.length);
      console.log("THE Y POSITION: " + action.y_position); // console.log("length of noteMessages" + NoteMessages.length)

      return Object.assign({}, state, {
        notes: Object(_home_joe_Documents_web_dev_sticky_notes_node_modules_babel_runtime_helpers_esm_toConsumableArray__WEBPACK_IMPORTED_MODULE_0__["default"])(state.notes).concat([{
          id: action.id,
          position: {
            x: posx,
            y: action.y_position
